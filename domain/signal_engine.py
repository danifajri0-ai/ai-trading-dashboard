...
def build_signal_summary(levels: MarketLevels, trend: TrendAnalysis, risk_plan: RiskPlan) -> SignalSummary:
    ...
    factors.append(_score_macd(levels.macd_histogram, trend.direction))
    ...
    action = "WAIT" if trend.direction == "WAIT" or confidence < 45 else trend.direction
...
def _score_macd(macd_histogram: float, direction: str) -> SignalFactor:
    if direction == "BUY":
        if macd_histogram > 0:
            return SignalFactor("MACD", 10, "Histogram MACD positif mendukung bias buy.")
        return SignalFactor("MACD", 4, "Histogram MACD belum mendukung bias buy.")
    if direction == "SELL":
        if macd_histogram < 0:
            return SignalFactor("MACD", 10, "Histogram MACD negatif mendukung bias sell.")
        return SignalFactor("MACD", 4, "Histogram MACD belum mendukung bias sell.")
    return SignalFactor("MACD", 5, "MACD belum memberi konfirmasi arah kuat.")
...
def signal_warnings(action: str) -> list[str]:
    if action == "WAIT":
        return ["Belum ada setup kuat, tunggu konfirmasi tambahan."]
    return []


def empty_data_warnings() -> list[str]:
    return ["Data kosong. Coba symbol/timeframe lain atau ulangi beberapa saat lagi."]
